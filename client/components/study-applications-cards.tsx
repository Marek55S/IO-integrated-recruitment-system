...
const CANCELLATION_POLICY_PAID =
...
function StudyApplicationsCards() {
  ...
  const handleCancelClick = (app: StudyApplication) => {
    if (app.status === 'awaiting_payment') {
      setCancelTarget({ app, variant: 'simple' });
    } else if (app.status === 'paid') {
      setCancelTarget({ app, variant: 'paid' });
    }
  };

  const handleConfirmCancel = () => {
    if (cancelTarget) {
      cancelStudyApplication(cancelTarget.app.id);
    }
  ...
  const dialogTitle = cancelTarget
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        confirmLabel="Tak, anuluj wniosek"
