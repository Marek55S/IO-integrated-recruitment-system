...
import { useLoadDataQuery, useSaveDataMutation } from "@/src/api/generated/graphql-api";
...
export default function Home() {
  ...
  const [saveData, { loading: saving, error: saveError }] = useSaveDataMutation();
  const {
    data: loadedData,
    refetch,
    loading: loadingData,
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-xl flex-col justify-center gap-6 p-6">
      ...
      <form onSubmit={onSave} className="space-y-4 rounded-lg border bg-card p-4">
        <div className="space-y-2">
          <Label htmlFor="input1">Input 1</Label>
          <Input id="input1" {...register("input1")} placeholder="Wpisz input1" />
        </div>

        <div className="space-y-2">
          <Label htmlFor="input2">Input 2</Label>
          <Input id="input2" {...register("input2")} placeholder="Wpisz input2" />
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Button type="button" variant="outline" onClick={onLoad} disabled={loadingData}>
            <Download />
      ...
      {loadError ? <p className="text-sm text-destructive">{loadError.message}</p> : null}
